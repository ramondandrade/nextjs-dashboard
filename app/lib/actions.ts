...
const CreateCustomer = FormSchemaCustomers.omit({ id: true });
...
/** CUSTOMERS */
export async function createCustomer(prevState: StateCustomers, formData: FormData) {
 ...
 try {
    await sql`
      ...
      VALUES (${name}, ${email}, '/customers/no-image.jpg')
    `;
  } catch (error) {
    // We'll also log the error to the console for now
    console.error(error);
